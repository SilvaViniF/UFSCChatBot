...
def get_formatted_input(messages, context):
    system = "Sistema: Esta é uma conversa entre um usuário e um assistente de inteligência artificial. O assistente fornece respostas úteis, detalhadas e educadas às perguntas do usuário com base no contexto. O assistente também deve indicar quando a resposta não pode ser encontrada no contexto."
    instruction = "Por favor, dê uma resposta completa e detalhada à pergunta."
...
def retrieval(messages):
    ...
    outputs = model.generate(quantization_config=bnb_config,input_ids=tokenized_prompt.input_ids, attention_mask=tokenized_prompt.attention_mask, max_new_tokens=128, eos_token_id=terminators)
